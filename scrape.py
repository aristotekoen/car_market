...
def scrape_ads_one_by_one():
    ids = pd.read_csv('data/processed_ads_df.csv', usecols=['id'])['id'].astype('str').tolist()
    scraped_ads = [os.path.splitext(os.path.basename(x))[0] for x in glob.glob('data/ads/*.json')]
    ...
    if failed_ads:
        np.save(arr=np.array(failed_ads), file='data/ads/failed_ads_crawl/failed_ads.npy')
