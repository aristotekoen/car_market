...
if not all([
    st.session_state["catboost_model_1"],
    st.session_state["catboost_model"],
    st.session_state["catboost_model_3"]
]):
...
registration_year = float(
    st.number_input("Registration year", min_value=2000, max_value=2025, format="%d")
)
fuel_type = st.selectbox("Fuel type", fuel_types, index=0)
gearbox_type = st.selectbox("Gearbox type", gearbox_types, index=1)
seats = float(
    st.slider("Number of seats", min_value=1, max_value=10, format="%d", value=5)
)
doors = float(
    st.number_input("Number of doors", min_value=1, max_value=10, format="%d", value=5)
)
rim_size = float(
    st.number_input(
        "Rim size (inches)?", min_value=10, max_value=30, help="Size of the wheel rims"
    )
)
drive_type = st.selectbox(
    "Drive type",
    drive_types,
    help="FWD: Front Wheel Drive, RWD: Rear Wheel Drive, AWD: All Wheel Drive, 4WD: four wheel drive",
)
...
never_crashed = st.selectbox(
    "Was the vehicle crashed in the past?", [True, False], index=1
)

st.header("🎨 Colors and interior material")
exterior_color = st.selectbox("Exterior color", exterior_colors)
interior_type = st.selectbox(
    "Interior type", interior_types, help="If unknown leave as is"
)
interior_color = st.selectbox(
    "Interior color", interior_colors, help="If unknown leave as is"
)
...
if st.button("Predict car price", use_container_width=True):
    ...
    with st.spinner("Estimating car price..."):
        ...
        price_q1, price_q2, price_q3 = predict_price(
            df_pool, catboost_model_1, catboost_model, catboost_model_3
        )
...
if "preds" in st.session_state and st.session_state["preds"] is not None:
    ...
    st.write("")
    st.metric(
        label="Reliability Score", value=f"{icon}  {int(reliability * 100)}%", delta=""
    )
...
list_features = [
    x
    for x in list(user_input.keys())
    if x not in ["lat", "lon", "is_new", "brand", "model", "never_crashed"]
]
user_input_effect = st.selectbox(
    "Select one characteristic and see how it could affect the value of your car!",
    list_features,
)
_lock = RLock()
with _lock:
    if st.button("Show the effect!"):
        ...
        df_input = pd.DataFrame(
            ...
            columns=st.session_state["df_input"].columns,
        )
        st.session_state["df_input"] = df_input
        ...
        if isinstance(df_input[user_input_effect].values[0], float) or isinstance(
            df_input[user_input_effect].values[0], int
        ):
            fig = pdp_num(
                df_input,
                test_set,
                user_input_effect,
                [catboost_model_1, catboost_model_2, catboost_model_3],
            )
